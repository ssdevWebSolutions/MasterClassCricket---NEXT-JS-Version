...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    document
      .querySelectorAll(`.${styles["headerHero-badge-number"]} span`)
      ...
      sections.forEach((section) => {
        ...
        if (
          ...
          const activeLink = document.querySelector(
            `.headerHero-nav-link[href="#${sectionId}"]`,
          );
          if (activeLink) {
  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://script.google.com/macros/s/AKfycbzoXbIhL0QvRt7x-jeqhAMlfYv0hY6_VzFf7EcUwiY0MswtuRnjax7fFuBF6U2GZmk3/exec",
        {
          method: "POST",
          body: formData,
        },
  ...
  return (
    <section className={styles["headerHero-section"]}>
              ...
              <div className={styles["headerHero-text-content"]}>
                <h1 className={styles["headerHero-title"]}>
                  <div className={styles["headerHero-title-container"]}>
                    <span className={styles["headerHero-title-top"]}>
                      𝙼𝙰𝚂𝚃𝙴𝚁𝙲𝙻𝙰𝚂𝚂 𝙲𝚁𝙸𝙲𝙺𝙴𝚃
                    </span>
                  </div>
                  <span className={styles["headerHero-title-main"]}>
                    FOR EVERY{" "}
                    <span className={styles["animated-skill"]}>"𝐒𝐊𝐈𝐋𝐋"</span>,
                  </span>
                  <span className={styles["headerHero-title-bottom"]}>
                    WE'VE GOT THE{" "}
                    <span className={styles["animated-drill"]}>"𝐃𝐑𝐈𝐋𝐋"</span>
                  </span>
                </h1>

                <p className={styles["headerHero-description"]}>
                  Masterclass Cricket is known for professional cricket coaching
                  backed by expert knowledge across all areas of the game. We
                  work with players at every level, from beginners to
                  international cricketers, empowering them to reach their full
                  potential and excel on the field.
                </p>
                ...
                <div className={styles["headerHero-cta-buttons"]}>
                  ...
                  >
                    <span className={styles["headerHero-btn-text"]}>
                      <span style={{ color: "blue" }}>Winter</span> Clinics
                      Registration
                    </span>{" "}
                ...
                <div className={styles["headerHero-achievement-badges"]}>
                  <div className={styles["headerHero-achievement-badge"]}>
                    ...
                    <span className={styles["headerHero-badge-text"]}>
                      Years Experience
                    </span>
                  </div>
                  <div className={styles["headerHero-achievement-badge"]}>
      ...
      {/* Gradient Effects */}
      <div
        className={`${styles["headerHero-gradient"]} ${styles["headerHero-gradient-1"]}`}
      ></div>
      <div
        className={`${styles["headerHero-gradient"]} ${styles["headerHero-gradient-2"]}`}
      ></div>
    </section>
  );
}
